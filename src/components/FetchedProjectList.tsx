...
function FeaturedProjectRender({
	...
	featuredProject: SanityDocument;
}) {
	return (
		<div className="flex flex-col xl:flex-row gap-8 group">
			<Link
				to={
					featuredProject._type === "portfolio-projects"
			...
			>
				<figure className="h-auto w-full aspect-video overflow-hidden shrink-0 ">
					<img
						src={urlFor(featuredProject.mainImage)
							.width(1000)
							.height(600)
							.url()}
			...
			>
				<div className="flex flex-col">
					<div className="flex flex-col gap-3">
						<div className="flex flex-col gap-1">
							...
							<p className="text-urg-orange p-small">
								{dateFormatter(featuredProject.publishedAt)}
							</p>
						</div>
					...
					<URGButton buttonType={ButtonType.expandArrow}>
						{featuredProject.coverVideo ? "Watch" : "Read more"}
					</URGButton>
				</div>
			</Link>
		</div>
			...
			<div className="flex flex-row xl:flex-col w-full gap-5 group">
				...
				<div className="w-full">
					<div className="flex flex-col gap-2">
						<div className="flex flex-col gap-1">
							<h4 className="group-hover:text-urg-blue">{project.title}</h4>
						...
						<p className="text-urg-black-75 xl:p-big">{project.description}</p>
					</div>
					<URGButton buttonType={ButtonType.expandArrow}>
...
export default function FetchedProjectList({
	...
	let featuredProject: SanityDocument | undefined;
	let otherProjects: SanityDocument[];
	if (highlightFeatured) {
		// 1. Find the specific "Featured Hero" (the first featured item)
		featuredProject = projectList.find((item) => item.featured);

		// 2. Get the rest of the items (excluding the one we picked for the Hero slot)
	...
	return (
		<div className="flex flex-col items-center mt-8">
			<div className="w-full xl:w-[84%] flex flex-col gap-16 xl:gap-24">
				{highlightFeatured && featuredProject ? (
					<FeaturedProjectRender featuredProject={featuredProject} />
				) : null}
				<div className="flex flex-col gap-8 xl:grid xl:grid-cols-3 xl:gap-24">
					{otherProjects && (
						<NonFeaturedProjectRender projects={otherProjects} />
					)}
				</div>
			</div>
		</div>
