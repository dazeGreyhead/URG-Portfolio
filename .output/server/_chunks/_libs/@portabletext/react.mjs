...
import { n as nestLists, L as LIST_NEST_MODE_HTML, i as isPortableTextToolkitList, a as isPortableTextListItemBlock, b as isPortableTextToolkitSpan, c as isPortableTextBlock, d as isPortableTextToolkitTextNode, s as spanToPlainText, e as buildMarksTree } from "./toolkit.mjs";
const defaultLists = {
  number: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("ol", { children }),
  bullet: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { children })
}, DefaultListItem = ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children }), link = ({ children, value }) => /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: value == null ? void 0 : value.href, children }), underlineStyle = { textDecoration: "underline" }, defaultMarks = {
  ...
  underline: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: underlineStyle, children }),
...
const hidden = { display: "none" }, DefaultUnknownType = ({
  ...
  return isInline ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: hidden, children: warning }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: hidden, children: warning });
}, DefaultUnknownMark = ({
  markType,
  children
}) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `unknown__pt__mark__${markType}`, children }), DefaultUnknownBlockStyle = ({
  children
}) => /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children }), DefaultUnknownList = ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { children }), DefaultUnknownListItem = ({
  children
}) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children }), DefaultHardBreak = () => /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}), defaultBlockStyles = {
...
function PortableText({
  ...
  const handleMissingComponent = missingComponentHandler || noop, blocks = Array.isArray(input) ? input : [input], nested = nestLists(blocks, listNestingMode || LIST_NEST_MODE_HTML), components = reactExports.useMemo(() => componentOverrides ? mergeComponents(defaultComponents, componentOverrides) : defaultComponents, [componentOverrides]), renderNode = reactExports.useMemo(
    () => getNodeRenderer(components, handleMissingComponent),
    [components, handleMissingComponent]
  ), rendered = nested.map(
    (node, index) => renderNode({ node, index, isInline: false, renderNode })
  );
  return /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: rendered });
}
const getNodeRenderer = (components, handleMissingComponent) => {
  ...
  function renderListItem(node, index, key) {
    const tree = serializeBlock({ node, index, isInline: false, renderNode }), renderer = components.listItem, Li = (typeof renderer == "function" ? renderer : renderer[node.listItem]) || components.unknownListItem;
    ...
    if (node.style && node.style !== "normal") {
      ...
      children = renderNode({ node: blockNode, index, isInline: false });
    }
    return /* @__PURE__ */ jsxRuntimeExports.jsx(Li, { value: node, index, isInline: false, renderNode, children }, key);
  }
  function renderList(node, index, key) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx(List, { value: node, index, isInline: false, renderNode, children }, key);
  }
  function renderSpan(node, _index, key) {
    const { markDef, markType, markKey } = node, Span = components.marks[markType] || components.unknownMark, children = node.children.map(
      (child, childIndex) => renderNode({ node: child, index: childIndex, isInline: true })
    );
    return Span === components.unknownMark && handleMissingComponent(unknownMarkWarning(markType), { nodeType: "mark", type: markType }), /* @__PURE__ */ jsxRuntimeExports.jsx(
      Span,
      {
  ...
  function renderBlock(node, index, key, isInline) {
    ...
    return Block === components.unknownBlockStyle && handleMissingComponent(unknownBlockStyleWarning(style), {
      nodeType: "blockStyle",
      type: style
    }), /* @__PURE__ */ jsxRuntimeExports.jsx(Block, { ...props, value: props.node, renderNode }, key);
  }
...
function serializeBlock(options) {
  const { node, index, isInline, renderNode } = options, children = buildMarksTree(node).map(
    (child, i) => renderNode({ node: child, isInline: true, index: i, renderNode })
  );
  return {
