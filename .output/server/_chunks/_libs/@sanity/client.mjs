import { o, e as ee, B, D, V, G, S as S$1, I, l } from "../../../_libs/get-it.mjs";
import { c as cjsExports, o as operatorsExports } from "../../../_libs/rxjs.mjs";
import { c as customAlphabet } from "../../../_libs/nanoid.mjs";
...
function getVersionId(id, version2) {
  ...
  return `${VERSION_PREFIX$1}${version2}${PATH_SEPARATOR$1}${getPublishedId$1(id)}`;
}
...
const pick = (obj, props) => props.reduce((selection, prop) => (typeof obj[prop] > "u" || (selection[prop] = obj[prop]), selection), {}), eventSourcePolyfill = cjsExports.defer(() => import("./eventsource.mjs").then(function(n) {
  return n.n;
})).pipe(
...
class LiveClient {
  ...
  events({
    ...
    const events = connectEventSource(() => (
      ...
      reconnectOnConnectionFailure(),
      cjsExports.mergeMap((event) => event.type === "reconnect" ? checkCors.pipe(cjsExports.mergeMap(() => cjsExports.of(event))) : cjsExports.of(event)),
      cjsExports.catchError((err) => checkCors.pipe(
        cjsExports.mergeMap(() => {
          throw err;
        })
      )),
...
class DatasetsClient {
  ...
  list() {
    ...
    const config = this.#client.config(), projectId2 = config.projectId;
    let uri = "/datasets";
    return config.useProjectHostname === false && (uri = `/projects/${projectId2}/datasets`), cjsExports.lastValueFrom(
...
class ProjectsClient {
  ...
  list(options) {
    const query = {}, uri = "/projects";
    return options?.includeMembers === false && (query.includeMembers = "false"), options?.organizationId && (query.organizationId = options.organizationId), cjsExports.lastValueFrom(_request(this.#client, this.#httpRequest, { uri, query }));
  }
  /**
   * Fetch a project by project ID
   *
...
class SanityClient {
  ...
  getDocument(id, options) {
    ...
    return cjsExports.lastValueFrom(_getDocument(this, this.#httpRequest, id, opts));
  }
  /**
   * Fetch multiple documents in one request.
   * Should be used sparingly - performing a query is usually a better option.
...
const stegaEncodeSourceMap$2 = /* @__PURE__ */ Object.freeze({
  __proto__: null,
  encodeIntoResult,
  stegaEncodeSourceMap,
